...
# 双向 LSTM模型定义
class DBiLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=2, dropout=0.3):
        super(DBiLSTMModel, self).__init__()
...
def main():
    ...
    model = DBiLSTMModel(input_size=1)
    print("训练 双向 LSTM 模型...")
    ...
    print("DBiLSTM 测试指标 (MSE, RMSE, MAE, R2, DA):", metrics)

    plt.figure(figsize=(10, 6))
    plt.plot(y_test_actual, label="真实值")
    plt.plot(preds, label="预测值")
    plt.title("双向 LSTM 模型测试集预测结果")
    plt.xlabel("样本索引")
    plt.ylabel("值")
    plt.legend()
    plt.grid(True)
